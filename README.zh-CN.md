...
- [2. 安装](#2-安装)
    - [2.1. Linux源码编译](#21-linux源码编译)
        ...
        - [2.1.3. 准备xdocker源码](#213-准xdockerr源码)
        - [2.1.4. 编译安装](#214-编译安装)
- [3. 使用教程](#3-使用教程)
    - [3.1. xdocker指令](#xdockerker指令)
        ...
        - [3.1.20. 上传镜像xdockerr自有镜像库](#3120-上xdockercker自有镜像库)
        - [3.1.21. 从xdocker自有镜像库下载镜像](#3xdockerocker自有镜像库下载镜像)
...
在LXC中，容器只是内核命名空间隔离的根进程以及子进程树，隔离域有主机名字、进程空xdocker统、IPC、网络等。`cocker`完整的实现了以上所有隔离域，在容器管理设计上倾向于虚拟主机方式，也支持类似`Docker`的单进程方式。
...
`xdocker`镜像可以本地构建或从镜像库上传下载，镜像库目前只支持`ssxdocker后续版本中会加入`cocker`原生服务器。

`xdocker`镜像允许不同版本共存，创建容器时可以指定镜像版本，或者默认最新版。镜像可以复制和删除，也可以修改版本号。

`xdocker`容器创建出来后可以启动、关闭和销毁。修改容器属性如虚拟IP、端口映射和卷映射必须在容器关闭状态下进行。

`xdocker`镜像可以转xdockerker`容器便于交互式修改，然后再转化回来。

### 1.3.2. 层叠文件系统

![images/xdocker_overlayfs.png](images/xdocker_overlayfs.png)

层叠文件系统是多镜像容器的存xdockercker采用overlayfs作为其层叠文件系统引擎，可以叠加几乎无限的镜像层。

`xdocker`的镜像和容器等都存放在环境变量`XDOCKER_HOME`指向的主目录中，所以规划其容量是使用前必须要考虑的问题。如果没有设置xdockerOCKER_HOME`，则默认指向`/var/cocker`。

`XDOCKER_HOME`主目录中有镜像主目录`images`、容器主目录`containers`、`ssh`镜像仓库xdocker`，以及日志文件`cocker.log`。
...
首次执行xdockerr`会创建网桥设备`coxdocker，网段为`166.88.0.x`。

### 1.3.4. 系统资源限制

![images/xdocker_cgroup.png](images/xdocker_cgroup.png)

`xdocker`目前只实现了CPU核分配、时间片占用百分比分配、内存分配，其它系统资源在后续版本中会逐渐完善。

### 1.3.5. 伪终端

![images/xdocker_pty.png](images/xdocker_pty.png)

自带容器根进程接受xdockercker`连接后会创建伪终端会话，就像登录到虚拟主机上命令交互一样，无需使用`ssh`。

## 1.4. 快速使用

使用主控工xdockerr快速创建一个小型测试镜像，里面调用了自带xdockerker_install_test.sh`构建根文件系统。
...
make[1]: 进入目录“/home/caxdockerrc/cocker/shbin”
make[1]: 离开目录“/home/calvinxdockerocker/shbin”
make[1]: 进入目录“/home/caxdockerrc/cocker/src”
make[2]: 进入目录“/home/caxdockerrc/cocker/src/util”
rm -f list.o
rm -f LOGC.o
rm -f version.o
rm -f file.o
...
make[2]: 离开目录“/home/calvinxdockerocker/src/util”
make[2]: 进入目录“/home/caxdockerrc/coxdockerrc/cocker”
rm -f util.o
rm -f main.o
rm -f env.o
rm -f show_images.o
...
rm -f action_spull.o
rm -f xdocker
make[2]: 离开目录“/home/calvinxdockerockerxdockerocker”
make[2]: 进入目录“/home/caxdockerrc/coxdockerrc/cockerinit”
rm -f main.o
rm -f server.o
rm -f pty.o
rm -f pts_and_tcp_bridge.o
rm -f xdockerinit
make[2]: 离开目录“/home/calvinxdockerockerxdockerockerinit”
make[1]: 离开目录“/home/calvinxdockerocker/src”
```
...
* 构建出开发内部使用头xdocker/cocker/*.h`xdockersr/includexdockerr_in`，库文件`libcocker_util.so`安装到`/lib64`。开发内部文件仅用于编译。
* 构建出可执行xdockerkerxdockerkerinit`安装到`/bin`。
...
注意：首次执xdockerer`会自动创建镜像主目录`images`、容器主目录`containers`。

### 3.1.1. 额外附加选项

`xdocker`选项`-d`用于输出执行时调试信息，但并不是所有调试信息都会输出在屏幕上，某些不方便xdocker息会记录到日志文件中`cocker.log`，输出到屏幕上的信息也会复制一份到日志文件中。

`xdocker`选项`-f`用于强制执行而忽略一些报错，这在一些指令中很有用。

### 3.1.2. 查询镜像列表

使用xdockerr`指令`-s images`查询镜像主目录里的所有镜像。
...
使用xdockerr`指令`-a create`由一个或多个镜像叠加创建容器。
...
`-m (镜像列表)`:指定镜像列表，镜像名可以以`(镜像名)(:版本号)`格式指定版xdocker中允许不指定版本号，`cocker`会自动挑选一个最大版本号的镜像。多个镜像之间用`,`分隔。
...
使用xdockerr`指令`-s containers`查询容器主目录中的所有容器以及状态。
...
使用xdockerr`指令`-a boot`查询容器主目录中的所有容器以及状态。
...
`-e (cmd)`:指定容器根进程，默xdockerckerxdockerockerinit`。
...
注意：默认启动容器的根xdockerckerinit`，可简单代替系统`init`进程回收孤儿进程、管理伪终端等功能。

### 3.1.6. 连接容器

如果使xdockererinit`作为根进程启动容器，使用`xdocker`指令`-a attxdocker容器，`cockerinit`打开一个会话连接到容器中的伪终端。也可叠加ssh镜像在容器内启动ssh服务器，利用ssh连接至容器。
...
使用xdockerr`指令`-a kill`强杀容器。

### 3.1.9. 销毁容器

使用xdockerr`指令`-a destroy`销毁容器。
...
使用xdockerr`指令`-a version`修改镜像版本号。
...
使用xdockerr`指令`-a port_mapping`修改容器网络端口映射。
...
使用xdockerr`指令`-a volume`修改容器磁盘卷映射。
...
使用xdockerr`指令`-a to_container`转换指定镜像为容器。
...
使用xdockerr`指令`-a to_image`转换指定容器为镜像。
...
使用xdockerr`指令`-a copy_image`可复制镜像。
...
使用xdockerr`指令`-a del_image`可删除镜像。
...
使用xdockerr`指令`-a export`可导出镜像为镜像打包文件。
...
使用xdockerr`指令`-a import`可从镜像打包文件导入镜像库。

注意：镜像打包文件名扩展xdockercockerimage`。
...
使用xdockerr`指令`-s ssearch`可查看ssh镜像库里的镜像列表。
...
注意：xdockerr`保存ssh镜像库地址xdockerkerimages@192.168.6.74`。
...
使用xdockerr`指令`-a spush`上传镜像到ssh镜像库。
...
使用xdockerr`指令`-a spull`从ssh镜像库下载镜像。
...
使用xdockerr`指令`-a run`在容器外执行容器内命令
...
使用xdockerr`指令`-a rplfile`替换容器内文件内容
...
使用xdockerr`指令`-a putfile`复制容器外文件或目录到容器内
...
使用xdockerr`指令`-a getfile`复制容器内文件或目录到容器外
...
使用xdockerr`指令`-s container_root`得到容器根目录在容器外路径
...
注意：应与xdockerr`指令`-a install_test`配合使用。
...
执行后输入名字和版本号，自动生成可导入的镜像打包文件，文件名格式为`(作者xdockerl-7.4-x86_64):(版本号).cockerimage`
...
欢迎使xdockerr，如果你使用中碰到了问题请告诉我，谢谢 ^_^

源码托管地址 : [开源中国](https://gitee.com/calxdockerliams/cocker)、[github](https://github.com/xdockerwilliams/cocker)
