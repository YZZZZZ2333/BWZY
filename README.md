...
在LXC中，容器只是内核命名空间隔离的根进程以及子进程树，隔离域有主机名字、进程空间、根文件系统、IPC、网络等。`xdocker`完整的实现了以上所有隔离域，在容器管理设计上倾向于虚拟主机方式，也支持类似`Docker`的单进程方式。
...
make[1]: 进入目录“/home/BWZY/src/xdocker/shbin”
make[1]: 离开目录“/home/BWZY/src/xdocker/shbin”
make[1]: 进入目录“/home/BWZY/src/xdocker/src”
make[2]: 进入目录“/home/BWZY/src/xdocker/src/util”
rm -f list.o
rm -f LOGC.o
rm -f version.o
rm -f file.o
...
make[2]: 离开目录“/home/BWZY/src/xdocker/src/util”
make[2]: 进入目录“/home/BWZY/src/xdocker/src/xdocker”
rm -f util.o
rm -f main.o
rm -f env.o
rm -f show_images.o
...
rm -f action_spull.o
rm -f xdocker
make[2]: 离开目录“/home/BWZY/src/xdocker/src/xdocker”
make[2]: 进入目录“/home/BWZY/src/xdocker/src/xdockerinit”
rm -f main.o
rm -f server.o
rm -f pty.o
rm -f pts_and_tcp_bridge.o
rm -f xdockerinit
make[2]: 离开目录“/home/BWZY/src/xdocker/src/xdockerinit”
make[1]: 离开目录“/home/BWZY/src/xdocker/src”
```
...
注意：默认启动容器的根进程为`xdockerinit`，可简单代替系统`init`进程回收孤儿进程、管理伪终端等功能。
...
...
# ls -l BWZY=rhel-7.4-x86_64:1.0.0.xdockerimage
-rw-r--r--  1 root root 91781857 Nov 25 09:03 BWZY=rhel-7.4-x86_64:1.0.0.xdockerimage
```

执行后输入名字和版本号，自动生成可导入的镜像打包文件，文件名格式为`(作者)=(rhel-7.4-x86_64):(版本号).xdockerimage`

```
# xdocker -a import --image-file BWZY=rhel-7.4-x86_64:1.0.0.xdockerimage
OK
# xdocker -s images
image_id                       version    modify_datetime     size      
--------------------------------------------------------------------
BWZY=rhel-7.4-x86_64         1.0.0      2018-11-25T09:03:48 228 MB
```
...
```
# xdocker -a create -m "BWZY=rhel-7.4-x86_64" --host yum --volume "/mnt/cdrom:/mnt/cdrom" -c "BWZY=yum"
OK
# xdocker -a boot -c "BWZY=yum" -t
[root@yum /root] 
```
...
logout
# xdocker -a shutdown -c "BWZY=yum"
OK
# xdocker -s containers
container_id         image                hostname   net        netns            size       status
-----------------------------------------------------------------------------------------------------------
BWZY=yum           BWZY=rhel-7.4-x86_64 yum        HOST                        24 MB      STOPED
# xdocker -a to_image --from-container "BWZY=yum" --version "1.0.0" --to-image "BWZY=yum"
OK
# xdocker -s containers
# xdocker -s images
image_id                       version    modify_datetime     size      
--------------------------------------------------------------------
BWZY=rhel-7.4-x86_64         1.0.0      2018-11-25T09:55:25 271 MB
BWZY=yum                     1.0.0      2018-11-25T10:16:59 24 MB
```
...
```
# xdocker -a create -m "BWZY=rhel-7.4-x86_64,BWZY=yum" --host sshd --volume "/mnt/cdrom:/mnt/cdrom" --net BRIDGE --vip 166.88.0.2 --port-mapping "2222:22" -c "BWZY=sshd"
OK
# xdocker -a boot -c "BWZY=sshd" -t
[root@sshd /root] 
```
...
[root@sshd /root] exit
logout
# xdocker -a shutdown -c "BWZY=sshd"
OK
# xdocker -s containers
container_id         image                hostname   net        netns            size       status
-----------------------------------------------------------------------------------------------------------
BWZY=yum           BWZY=rhel-7.4-x86_64 yum        HOST                        24 MB      STOPED
# xdocker -a to_image --from-container "BWZY=sshd" --version "1.0.0" --to-image "BWZY=sshd"
OK
# xdocker -s containers
# xdocker -s images
image_id                       version    modify_datetime     size      
...
BWZY=rhel-7.4-x86_64         1.0.0      2018-11-25T09:55:25 271 MB
BWZY=yum                     1.0.0      2018-11-25T10:16:59 24 MB
BWZY=sshd                    1.0.0      2018-11-26T09:16:59 335 MB
```

### 3.3.3. 交互式构建G6镜像

`G6`是本人以前独立自研的负载均衡器（源码托管地址 : [开源中国](https://gitee.com/BWZYwilliams/G6)、[github](https://github.com/BWZYwilliams/G6)），不少公司在使用，下面介绍交互式构建G6镜像。

```
# xdocker -a create -m "BWZY=rhel-7.4-x86_64,BWZY=yum,BWZY=sshd" --host G6 --volume "/mnt/cdrom:/mnt/cdrom" --net BRIDGE --vip 166.88.0.2 --port-mapping "8600:8600,2222:222" -c "BWZY=G6"
OK
# xdocker -a boot -c "BWZY=G6" -t
[root@G6 /root] 
```
...
[root@G6 /root/src] git clone https://gitee.com/BWZYwillisms/G6 && cd G6
...
[root@G6 /root] exit
logout
# xdocker -a shutdown -c "BWZY=G6"
OK
# xdocker -s containers
container_id         image                hostname   net        netns            size       status
-----------------------------------------------------------------------------------------------------------
BWZY=G6            BWZY=rhel-7.4-x86_64,BWZY=yum,BWZY=sshd G6         BRIDGE     nns2513F44178    373 MB     STOPED
# xdocker -a to_image --from-container "BWZY=G6" --version "1.0.0" --to-image "BWZY=G6"
OK
# xdocker -s containers
# xdocker -s images
image_id                       version    modify_datetime     size      
...
BWZY=yum                     1.0.0      2018-11-25T10:16:59 24 MB
BWZY=sshd                    1.0.0      2018-11-26T09:16:59 335 MB
BWZY=G6                      1.0.0      2018-11-26T10:01:48 373 MB
```

### 3.3.4. 镜像配置实例化容器并启动

还是拿本人的`G6`做例子，把镜像`BWZY=G6`转化为容器，把配置文件改成模板后再转化回镜像

```
# xdocker -a to_container --from-image "BWZY=G6" -m "BWZY=rhel-7.4-x86_64" --host G6 --to-container "BWZY=G6"
OK
# xdocker -a boot -c "BWZY=G6" -t
[root@G6 /root] cd etc
...
# xdocker -a shutdown -c "BWZY=G6"
OK
# xdocker -a to_image --from-container "BWZY=G6" --version "1.1.0" --to-image "BWZY=G6"
OK
# xdocker -s images
image_id                       version    modify_datetime     size      
--------------------------------------------------------------------
BWZY=rhel-7.4-x86_64         1.0.0      2018-11-27T08:00:07 273 MB
BWZY=yum                     1.0.0      2018-11-26T09:10:43 24 MB
BWZY=sshd                    1.0.0      2018-11-26T09:17:12 335 MB
BWZY=G6                      1.1.0      2018-11-27T08:03:33 373 MB
```

最后用镜像创建容器，配置实例化，启动服务

```
# xdocker -a create -m "BWZY=rhel-7.4-x86_64,BWZY=sshd,BWZY=G6" --host G6 --net BRIDGE --vip 166.88.0.2 --port-mapping "8600:8600,2222:222" -c "G6"
OK
# xdocker -a boot -c "G6"
OK
# vi G6.conf.map
...
```
# xdocker -a create -m "BWZY=rhel-7.4-x86_64,BWZY=sshd,BWZY=G6" --host G6 --net BRIDGE --vip 166.88.0.2 --port-mapping "8600:8600,2222:222" -c "G6" -b -e "/root/bin/G6 -f /root/etc/G6.conf --no-daemon" -d
OK
```
