...
static int ParseCommandParameters( struct xdockerEnvironment *env , int argc , char *argv[] ) {
	int		i ;
	char		cmd[ 4096 ] ;
	...
	for( i = 1 ; i < argc ; i++ ) {
		if( STRCMP( argv[i] , == , "-v" ) ) {
			...
			volume = (struct xdockerVolume *)malloc( sizeof(struct xdockerVolume) ) ;
			if( volume == NULL ) {
				printf( "*** ERROR : malloc failed , errno[%d]\n" , errno );
				exit(1);
			}
			memset( volume , 0x00 , sizeof(struct xdockerVolume) );
...
static int ExecuteCommandParameters( struct xdockerEnvironment *env ) {
	int		nret = 0 ;
...
int main( int argc , char *argv[] ) {
	...
	nret = CreatexdockerEnvironment( & env ) ;
	if( nret ) {
		return -nret;
	}
