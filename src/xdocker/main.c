...
static void usage()
{
	printf( "使用方法 : xdocker -v\n" );
	printf( "查询镜像列表：  -s images\n" );
	printf( "查询容器列表：  -s containers\n" );
	printf( "创建容器：      -a create (-m|--image) (image[:version])[,(image[:version])]... [ create options ] [ (-c|--container) (container) ] [ (-b|--boot) [ cgroup options ] [ (-t|--attach) | (-e|--exec) (cmd|\"program para1 ...\") ] ]\n" );
	printf( "文件内容替换：  -a rplfile (-c|--container) (container) (--template-file) (template_file) (--mapping-file) (mapping_file) [ (--instance-file) (instance_file) ]\n" );
	printf( "开启容器：      -a boot (-c|--container) (container) [ cgroup options ] [ (-t|--attach) | (-e|--exec) (cmd|\"program para1 ...\") ]\n" );
	printf( "连接到容器：    -a attach (-c|--container) (container)\n" );
	printf( "运行容器命令：  -a run (-c|--container) (container) (--cmd) (cmd)\n" );
	printf( "关闭容器：      -a shutdown (-c|--container) (container) [ (-f|--forcely) ]\n" );
	printf( "强行停止：      -a kill (-c|--container) (container) [ (-f|--forcely) ]\n" );
	printf( "删除容器：      -a destroy (-c|--container) (container) [ (-f|--forcely) ] [ (-h|--shutdown) ]\n" );
	printf( "查询版本：      -a version (-m|--image) (image[:version]) [ --version (version) ]\n" );
	printf( "修改容器IP：    -a vip (-c|--container) (container) --vip (ip)\n" );
	printf( "端口映射：      -a port_mapping (-c|--container) (container) --port-mapping (src_port:dst_port)\n" );
	printf( "分卷管理：      -a volume (-c|--container) (container) --volume (host_path[:container_path])[ ...]\n" );
	printf( "容器转镜像：    -a to_image --from-container (container) [ --verion (verion) ] --to-image (image)\n" );
	printf( "镜像转容器：    -a to_container --from-image (image[:version]) (-m|--image) (image[:version])[,(image[:version])]... [ create options ] --to-container (container)\n" );
	printf( "复制镜像：      -a copy_image --from-image (image[:version]) --to-image (image[:version])\n" );
	printf( "删除镜像：      -a del_image (-m|--image) (image[:version])\n" );
	printf( "导入镜像：      -a import --image-file (file)\n" );
	printf( "导出镜像：      -a export (-m|--image) (image[:version])\n" );
	printf( "搜索镜像：      -s ssearch --srepo (user@host)\n" );
	printf( "安装测试：      -a install_test\n" );
...
static int ParseCommandParameters( struct CockerEnvironment *env , int argc , char *argv[] )
{
	...
	else
	{
		...
		while( ifa )
		{
			...
			{
				if( STRCMP( ifa->ifa_name , != , "lo" ) && STRCMP( ifa->ifa_name , != , "xdocker0" ) && STRNCMP( ifa->ifa_name , != , "veth" , 4 ) )
				{
