...
int DoShow_containers( struct xdockerEnvironment *xdocker_env ) {
	DIR		*dir = NULL ;
	...
	dir = opendir( xdocker_env->containers_path_base ) ;
	count = 0 ;
	while(1) {
		dirent = readdir( dir ) ;
		if( dirent == NULL )
		...
		if( Snprintf( container_path_base , sizeof(container_path_base) , "%s/%s" , xdocker_env->containers_path_base , dirent->d_name ) == NULL )
			continue;
		...
		nret = ReadFileLine( image , sizeof(image) , container_image_file , sizeof(container_image_file) , "%s/%s/image" , xdocker_env->containers_path_base , dirent->d_name ) ;
		if( nret ) {
			memset( container_image_file , 0x00 , sizeof(container_image_file) );
			memset( image , 0x00 , sizeof(image) );
		}
		...
		nret = ReadFileLine( hostname , sizeof(hostname) , container_hostname_file , sizeof(container_hostname_file) , "%s/%s/hostname" , xdocker_env->containers_path_base , dirent->d_name ) ;
		if( nret ) {
			memset( container_hostname_file , 0x00 , sizeof(container_hostname_file) );
			memset( hostname , 0x00 , sizeof(hostname) );
		}
		...
		nret = ReadFileLine( net , sizeof(net) , container_net_file , sizeof(container_net_file) , "%s/%s/net" , xdocker_env->containers_path_base , dirent->d_name ) ;
		if( nret ) {
			memset( container_net_file , 0x00 , sizeof(container_net_file) );
			memset( net , 0x00 , sizeof(net) );
		}
		...
		nret = ReadFileLine( netns , sizeof(netns) , container_netns_file , sizeof(container_netns_file) , "%s/%s/netns" , xdocker_env->containers_path_base , dirent->d_name ) ;
		if( nret ) {
			memset( container_netns_file , 0x00 , sizeof(container_netns_file) );
			memset( netns , 0x00 , sizeof(netns) );
		}
		...
		/* size */
		Snprintf( container_rwlayer_path , sizeof(container_rwlayer_path) , "%s/%s/rwlayer" , xdocker_env->containers_path_base , dirent->d_name );
		
		memset( file_buffer , 0x00 , sizeof(file_buffer) );
		memset( container_size_file , 0x00 , sizeof(container_size_file) );
		nret = ReadFileLine( file_buffer , sizeof(file_buffer) , container_size_file , sizeof(container_size_file) , "%s/%s/size" , xdocker_env->containers_path_base , dirent->d_name ) ;
		if( nret ) {
			...
			if( nret ) {
				...
				WriteFileLine( file_buffer , container_size_file , sizeof(container_size_file) , "%s/%s/size" , xdocker_env->containers_path_base , dirent->d_name );
			}
		}
		else {
		...
		nret = ReadFileLine( pid_str , sizeof(pid_str) , container_pid_file , sizeof(container_pid_file) , "%s/%s/pid" , xdocker_env->containers_path_base , dirent->d_name ) ;
		if( nret ) {
			memset( pid_str , 0x00 , sizeof(pid_str) );
			memset( container_pid_file , 0x00 , sizeof(container_pid_file) );
			if( nret > 0 ) {
