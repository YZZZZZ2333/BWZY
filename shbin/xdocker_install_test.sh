...
if [ $# -ne 1 ] ; then
	echo "USAGE : xdocker_install_test.sh (image_path_base)"
	exit 1
fi
...
cp xdockerinit ${IMAGE_RLAYER_PATH_BASE}/bin/
cd ${IMAGE_RLAYER_PATH_BASE}/bin/
xdocker_ldd_and_cp_lib64.sh
...
cp /bin/xdocker_etc_profile_template.sh ${IMAGE_RLAYER_PATH_BASE}/etc/profile
cp /bin/xdocker_profile_template.sh ${IMAGE_RLAYER_PATH_BASE}/root/.profile
